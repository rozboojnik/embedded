...
#define COMPRESSED_MEM               // If defined, use seperate memory spaces
                                     // for data and code. Slower but compact
...
#if defined COMPRESSED_MEM
	tU8 ram[0x0200];
	tU8 ramHigh[M6502_MEM_SIZE];
#else
	tU8 ram[M6502_MEM_SIZE]; // Memory for the M6502 simulator
#endif
tU8 board[64];           // Copy of the chess board from 6502 memory
                         // Used to check for fields that need a redraw
M6502 cpuRegs;
M6502 *R;

tU8 halt;                // Flag to identify if simulation should be stopped
tU8 border;              // Border (background) color used as color
tU8 oldTo, oldFrom;
 ...
 *    Do not show the move if it's illegal (during startup).
 *
 * Params:
